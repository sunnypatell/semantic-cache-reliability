...
Our findings are not reassuring for the default of trusting a single similarity
threshold. \pending{headline: on adversarial paraphrases the strongest embedding
separates equivalent from non-equivalent pairs barely above chance}. \pending{headline:
to hold the false-hit rate under one percent, even the best model serves only a small
fraction of queries}. \pending{headline: false hits degrade downstream task quality by
X, and the damage depends on where in the pipeline the cache sits}. \pending{headline:
cost-aware calibration shifts the cost-coverage frontier relative to fixed and
correctness-bounding thresholds, and transfers across domains with a small gap}.
...
\begin{itemize}
  ...
  \item A cost-aware threshold-calibration policy that ties the operating point to the
        measured cost of a false hit and transfers across models and domains without
        online feedback, with an honest comparison to fixed and correctness-bounding
        baselines (\cref{sec:calibration}).
\end{itemize}
