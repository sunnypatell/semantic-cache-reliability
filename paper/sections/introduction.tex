...
\begin{figure}[t]
  ...
  \caption{\textbf{Cache-equivalence is not cosine similarity.} \textbf{(a)}~A semantic cache
    serves the stored answer to any prompt whose embedding lies within a cosine threshold
    $\thr$ of a cached prompt (shaded). The genuine paraphrase ``merits of X?'' and the
    near-duplicate ``weaknesses of X?'' sit equally close to the cached ``strengths of X?'',
    yet only the first may reuse the stored answer; no threshold admits the true hit while
    rejecting the false one. \textbf{(b)}~Because the cache fires on similarity alone, the
    false hit leaves as a confident, unlogged wrong answer. This paper measures how often this
    happens across encoders and domains, what it costs downstream, and how to price it
    into~$\thr$.}
  \Description{Two panels. Panel (a) shows an embedding space with a dashed circle marking a
    cosine similarity threshold around a cached prompt; a genuine paraphrase and a
    word-scrambled near-duplicate both fall inside the circle, but only one shares the cached
    answer, so a threshold cannot separate them. Panel (b) shows a pipeline in which a query is
    encoded, matched to the nearest cached entry, and served whenever similarity clears the
    threshold, with the false hit leaving as a confident wrong answer.}
  \label{fig:teaser}
\end{figure}
...
(\cref{fig:teaser}): a semantic cache fails exactly where the two diverge.
