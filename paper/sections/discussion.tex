...
edges. Two duplicate questions can warrant slightly different answers in context, and two
non-paraphrases can occasionally share an answer. We accept this gap deliberately, because
the alternative, bespoke answer-equivalence annotation at scale, would trade a transparent,
reproducible label for a private one. The direction of the bias also works in our favor for
the central claim: \mbox{PAWS} pairs are labeled by humans as \emph{non}-paraphrases despite
near-identical wording, so counting a high-similarity \mbox{PAWS} non-paraphrase as a
potential false hit is conservative, not generous. The numbers should be read as the
reliability of the similarity proxy against a human notion of sameness, which is the proxy a
deployed cache actually relies on.
...
deployment can measure with this protocol.

\paragraph{Conclusion validity.}
All intervals are $95\%$ bias-corrected and accelerated bootstrap intervals over pairs, the
many comparisons are corrected for the false discovery rate, and the analysis plan was
preregistered before the main run. Effect sizes accompany the significance tests so that a
reliable difference is not mistaken for a large one.
