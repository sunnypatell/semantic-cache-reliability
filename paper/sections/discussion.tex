...
Cosine scores are not comparable across encoders, so every between-encoder statement uses
...
cases: a conservative, cost-aware threshold, and, where the latency budget allows, a
verification step rather than blind trust in similarity. Our cross-encoder measurement tempers
the second lever: a joint verifier sharpens the easy and middling cases but does not recover
adversarial near-duplicates (\cref{sec:results}), so on the hardest prompts the dependable
choice is to decline to cache. On the other side of the ledger, caching reduces the compute and energy
of repeated inference, and a more reliable cache captures that saving without silently
degrading the answers it serves.
