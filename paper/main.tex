...
On adversarial near-duplicate prompts every embedding, including the largest, separates
equivalent from non-equivalent pairs barely above chance, and a lexical baseline beats them
all; holding the false-hit rate under one percent caps coverage near nine percent on the
easiest domain and under one percent on the rest. A fixed threshold proves catastrophic
under a realistic false-hit penalty, costing several times the price of not caching; a
cost-aware threshold tied to the measured penalty recovers near-baseline cost, but a value
calibrated on an easy domain is ruinous on a hard one. The results caution against trusting a
single similarity threshold, and they give practitioners a principled way to set one.
\end{abstract}
